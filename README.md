...
- **四國語言介面**：提供繁體中文、English、日本語、한국어 四種語言切換，透過頁面上方的醒目按鈕即可一鍵切換。
- **今日吉時速查**：自動計算當日十二時辰的卦象，以綠色（吉）與紅色（凶）卡片一目了然呈現，並以星號標示當前時辰。
...
3. **切換介面語言**
   頁面上方提供四個語言按鈕（🇹🇼 繁中 / 🇺🇸 EN / 🇯🇵 日本語 / 🇰🇷 한국어），點擊即可切換整個介面的語言，包含所有解籤說明文字。語言偏好會自動記憶。
4. **今日吉時速查**
   占卜表單下方會自動顯示當日十二個時辰的吉凶卡片。綠色代表吉（大安、速喜、小吉），紅色代表凶（留連、赤口、空亡），當前時辰會以「★」標示並加粗。修改農曆月份或日期後卡片會即時更新。
5. **複製與查閱紀錄**
